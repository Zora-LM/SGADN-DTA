...
def parse_args():
    ...
    parser.add_argument('--processed_dir', type=str, default='/media/data0/lm/Experiments/3D_DTI/GeoTrans_pool_v2/dataset/processed/', help='Preprocessed dataset')
    ## dataset prepare
    ...
    parser.add_argument("--embed_type", type=str, default='int', help='int, float, both')
...
if __name__ == '__main__':
    ...
    # Save path
    args.save_dir = args.save_dir.format(args.split_type, args.transformer, args.n_layer, args.cutoff, args.graph_pool,
    ...
    for repeat in range(args.n_repeat):
        ...
        train_dataset = MoleculeDataset(graph_path=f'{args.processed_dir}/{args.dataset}/{args.split_type}/repeat{repeat}_train_{args.cutoff}_{args.n_angle}_graph.pkl')
        val_dataset = MoleculeDataset(graph_path=f'{args.processed_dir}/{args.dataset}/{args.split_type}/repeat{repeat}_val_{args.cutoff}_{args.n_angle}_graph.pkl')
        test_dataset = MoleculeDataset(graph_path=f'{args.processed_dir}/{args.dataset}/{args.split_type}/test_{args.cutoff}_{args.n_angle}_graph.pkl')
        if args.dist_train:
            ...
            val_loader = DataLoader(val_dataset, sampler=DistributedSampler(val_dataset),
                                      batch_size=args.batch_size, num_workers=args.n_threads,
                                      worker_init_fn=seed_worker, drop_last=False, collate_fn=collator_test)
