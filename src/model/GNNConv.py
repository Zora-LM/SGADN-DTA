...
class GNNlayer(nn.Module):
    def __init__(self,
        ...
        if self.args.alpha_cal:
            self.alpha_cal = alpha_calculation(dim=out_feats)
    ...
    def forward(self, graph: DGLGraph, ent_embed, rel_embed=None, drop_edge_ids=None):
        ...
        if self.args.diffusion:
            ent_rst = self.ppr_estimation(graph=graph)  # PageRankP
    ...
    def ppr_estimation(self, graph: DGLGraph):
        ...
        if self.args.alpha_cal:
            self.alpha = self.alpha_cal(feat)
            beta = 1e-6
            self.alpha.clamp(min=beta, max=1 - 1e-6)
            # print(self.alpha)
